# Document AI Extraction Quality Report: `sternberg_review_2nd_p0001_p0015_pilot`

**Audit Status:** ✅ PASSED (100% PROVENANCE)  
**Parent Document:** Sternberg's Diagnostic Surgical Pathology Review (`ae721928-8248-5cca-9499-29264a91656e`)  
**Original Page Span:** Pages 1 – 15 (15 pages)  
**Generated At:** 2026-08-30T14:56:08.045483+00:00  

---

## 1. Provenance Verification

| Provenance Metric | Value | Threshold / Target | Status |
|---|---|---|---|
| **Provenance Integrity Score** | **100.0%** | 100.0% | ✅ Verified |
| **Blocks with Parent Doc ID** | 230 / 230 | 100% | ✅ OK |
| **Blocks with 1-based Book Page** | 230 / 230 | 100% | ✅ OK |
| **Page Offset Bound Check** | 1 <= p <= 15 | In Range | ✅ OK |

---

## 2. Statistical & Structural Metrics

| Metric | Count / Score | Details |
|---|---|---|
| **Total Extracted Blocks** | 230 | Structured units |
| **Total Word Count** | 1,145 | Extracted tokens |
| **Total Character Count** | 7,540 | Normalized characters |
| **Average Layout Confidence** | **92.31%** | Layout Parser OCR |
| **Low Confidence Blocks (<75%)** | 0 | Flagged for review |

### Structural Element Distribution
- **Headings (H1/H2/H3):** 18
- **Paragraphs:** 212
- **Tables:** 0
- **Lists / Bullets:** 0
- **Figure Captions:** 0

---

## 3. Anomaly & Quality Findings

- *None detected. Extraction clean and verified.*
