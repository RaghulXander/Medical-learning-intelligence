...
export default function QuestionEditorPage({ params }: { params: { questionId: string } }) {
  ...
  if (authLoading || loading) return <div className="min-h-[60vh] grid place-items-center text-foreground/80">Loading question editor…</div>;
  if (!canEdit) return <div className="min-h-[60vh] grid place-items-center"><Card className="glass-card p-8 text-center"><ShieldAlert className="mx-auto mb-3 text-amber-300" /><p className="text-foreground">Editorial permission required.</p></Card></div>;
  ...
  return <div className="container mx-auto max-w-7xl px-4 py-8">
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <div className="space-y-6">
        {canReview && <Card className="glass-card p-5"><h2 className="mb-3 font-bold text-foreground">Review workflow</h2><textarea rows={4} value={reviewNotes} onChange={(event) => setReviewNotes(event.target.value)} placeholder="Review notes (required for reject/retire)" className="mb-3 w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground" /><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={saving || !allowed.includes('HUMAN_REVIEW')} onClick={() => void transition('HUMAN_REVIEW')}>Human review</Button><Button size="sm" variant="gradient" disabled={saving || !allowed.includes('APPROVED')} onClick={() => void transition('APPROVED')}>Approve</Button><Button size="sm" variant="destructive" disabled={saving || !allowed.includes('REJECTED')} onClick={() => void transition('REJECTED')}>Reject</Button><Button size="sm" variant="outline" disabled={saving || !allowed.includes('RETIRED')} onClick={() => void transition('RETIRED')}>Retire</Button></div></Card>}
        <Card className="glass-card p-5"><h2 className="mb-3 flex items-center gap-2 font-bold text-foreground"><History className="h-4 w-4" />Revision history</h2>{revisions.length === 0 ? <p className="text-xs text-muted-foreground">No content revisions yet.</p> : <div className="space-y-3">{revisions.map((revision) => <div key={revision.id} className="rounded-lg border border-border p-3 text-xs"><div className="font-semibold text-foreground">Revision {revision.revision_number}</div><div className="mt-1 text-muted-foreground">{revision.changed_fields.join(', ')}</div>{revision.edit_notes && <p className="mt-2 text-foreground/80">{revision.edit_notes}</p>}<time className="mt-2 block text-muted-foreground/80">{new Date(revision.created_at).toLocaleString()}</time></div>)}</div>}</Card>
        {question.citations && question.citations.length > 0 && <Card className="glass-card p-5"><h2 className="mb-3 font-bold text-foreground">Evidence (read only)</h2>{question.citations.map((citation, index) => <div key={index} className="mb-2 rounded border border-border p-3 text-xs text-foreground/80">{citation.source_title}<br /><span className="text-muted-foreground/80">{citation.verification_status}</span></div>)}</Card>}
      </div>
    </div>
  </div>;
}
