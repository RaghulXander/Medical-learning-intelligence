...
export default function MobileLayoutEditorPage() {
  ...
  if (authLoading || loading) {
    return <div className="min-h-[60vh] grid place-items-center text-slate-300">Loading mobile layout…</div>;
  }
  if (!isAdmin) {
    return <div className="min-h-[60vh] grid place-items-center"><Card className="glass-card p-8 text-center"><ShieldAlert className="mx-auto mb-3 text-amber-300" /><p className="text-white">Mobile layout administrator access required.</p></Card></div>;
  }
    ...
    <div className="container mx-auto max-w-7xl px-4 py-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-3xl font-bold text-white">Native Home Layout</h1>
        ...
        <Card className="glass-card h-fit p-3">
          <div className="space-y-2">
            {widgets.map((widget, index) => (
              <button key={widget.id} type="button" onClick={() => setActiveId(widget.id)} className={`w-full rounded-lg border p-3 text-left ${active?.id === widget.id ? 'border-sky-500/50 bg-sky-500/10' : 'border-white/10'}`}>
                <div className="flex justify-between text-sm font-semibold text-white"><span>{mobileWidgetLabels[widget.type]}</span><span>{widget.enabled ? 'On' : 'Off'}</span></div>
          ...
          <div className="mt-4 flex gap-2 border-t border-white/10 pt-4">
            <select value={addType} onChange={(event) => setAddType(event.target.value as MobileWidgetType)} className="min-w-0 flex-1 rounded bg-slate-950 text-xs text-white">{(Object.keys(mobileWidgetLabels) as MobileWidgetType[]).map((type) => <option key={type} value={type}>{mobileWidgetLabels[type]}</option>)}</select>
        ...
        <div className="space-y-6">
          {active && (
            <Card className="glass-card p-5">
              <div className="mb-5 flex justify-between border-b border-white/10 pb-4"><div><h2 className="text-xl font-bold text-white">{mobileWidgetLabels[active.type]}</h2><p className="text-xs text-slate-400">{active.id}</p></div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => updateWidget(active.id, (widget) => ({ ...widget, enabled: !widget.enabled }))}>{active.enabled ? 'Hide' : 'Show'}</Button><Button size="sm" variant="destructive" disabled={widgets.length <= 1} onClick={() => replaceWidgets(widgets.filter((widget) => widget.id !== active.id))}>Remove</Button></div></div>
              <div className="mb-5 grid gap-4 sm:grid-cols-3">
                <label className="text-xs text-slate-300">Audience<select value={active.audience} onChange={(event) => updateWidget(active.id, (widget) => ({ ...widget, audience: event.target.value as MobileWidget['audience'] }))} className="mt-1 w-full rounded bg-slate-950 p-2 text-white"><option value="ALL">All</option><option value="AUTHENTICATED">Authenticated</option><option value="FREE">Free</option><option value="SUBSCRIBED">Subscribed</option></select></label>
                <label className="text-xs text-slate-300">Platform<select value={active.platforms[0]} onChange={(event) => updateWidget(active.id, (widget) => ({ ...widget, platforms: [event.target.value as 'ALL' | 'IOS' | 'ANDROID'] }))} className="mt-1 w-full rounded bg-slate-950 p-2 text-white"><option value="ALL">All</option><option value="IOS">iOS</option><option value="ANDROID">Android</option></select></label>
                <label className="text-xs text-slate-300">Rollout %<input type="number" min="0" max="100" value={active.rolloutPercentage} onChange={(event) => updateWidget(active.id, (widget) => ({ ...widget, rolloutPercentage: Number(event.target.value) }))} className="mt-1 w-full rounded bg-slate-950 p-2 text-white" /></label>
              </div>
          ...
          <Card className="glass-card p-5"><label className="text-xs font-semibold text-slate-300">Publication notes<textarea rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} className="mt-2 w-full rounded bg-slate-950 p-3 text-sm text-white" /></label><p className="mt-3 text-xs text-slate-400">The app renders only precompiled widget types. Invalid configuration uses the bundled fallback layout.</p></Card>
        </div>
      </div>
    </div>
  );
