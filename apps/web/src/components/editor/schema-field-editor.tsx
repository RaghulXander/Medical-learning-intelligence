...
function FieldControl({ field, value, path, onChange }: { field: FieldDefinition; value: JsonValue | undefined; path: Array<string | number>; onChange: SchemaFieldEditorProps['onChange'] }) {
  const controlClass = 'w-full rounded-md border border-border bg-background/60 px-3 py-2 text-sm text-foreground';
  ...
  if (field.type === 'boolean') return <button type="button" onClick={() => onChange(path, value !== true)} className={`rounded-full px-3 py-1 text-xs font-semibold ${value === true ? 'bg-emerald-500/20 text-emerald-300' : 'bg-muted text-foreground/80'}`}>{value === true ? 'Enabled' : 'Disabled'}</button>;
...
export function SchemaFieldEditor({ value, fields, onChange, path = [] }: SchemaFieldEditorProps) {
  return <div className="space-y-4">{fields.map((field) => <div key={field.name}>
    <label className="mb-1.5 block text-xs font-semibold text-foreground/80">{field.label}{field.required ? ' *' : ''}</label>
    {field.description && <p className="mb-1.5 text-[11px] text-muted-foreground/80">{field.description}</p>}
